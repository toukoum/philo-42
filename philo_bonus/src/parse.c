...
/*   Updated: 2024/02/06 05:04:50 by rgiraud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_semaphore(t_table *table)
{
	...
	if (table->sem_log == SEM_FAILED)
		return (INIT_ERR);
	table->sem_eat_full = sem_open("/eat_full", O_CREAT, 0644,
			0);
...
int init_process_philo(t_philo **philo, t_table *table, size_t i)
{
    *philo = malloc(sizeof(t_philo));
    ...
    (*philo)->sem_count_meal = sem_open("/count_meal", O_CREAT, 0644, 1);
    if ((*philo)->sem_count_meal == SEM_FAILED)
        return (INIT_ERR);
    (*philo)->sem_last_meal = sem_open("/last_meal", O_CREAT, 0644, 1);
